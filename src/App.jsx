...
const MULTI_ROLES = ["Líder","Secretário","Anfitrião","Supervisor","Líder em Treinamento","Visitante"]
...
function getNextMeetingDate(day,frequency){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.85)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:460,maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"16px 20px",borderTop:"1.5px solid #f1f5f9",background:"#fff"}}>
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      <header style={{background:"#0f172a",padding:"14px 18px 0",position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",gap:1,overflowX:"auto",paddingBottom:2}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{background:tab===t.id?"rgba(37,99,235,0.85)":"transparent",border:"none",borderRadius:"8px 8px 0 0",padding:"7px 10px 10px",cursor:"pointer",color:tab===t.id?"#fff":"#94a3b8",fontSize:10,fontWeight:700,display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:50,flexShrink:0}}>
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        <Stat label="Células Ativas" value={activeCells.length} color="#2563eb" icon="grid"/>
      ...
      {nextMeetings.length>0&&(
        <Card style={{marginBottom:14,border:"1.5px solid #bfdbfe",background:"#eff6ff"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <span style={{fontSize:14,fontWeight:800,color:"#1e40af"}}>📅 Próximos Encontros</span>
          </div>
          {nextMeetings.map(c=>(
            <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderTop:"1px solid #bfdbfe"}}>
              <span style={{fontSize:13,fontWeight:700,color:"#1e40af"}}>{c.name}</span>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:12,fontWeight:700,color:"#1e40af"}}>{fmtDate(c.next_meeting_date)}</div>
                <div style={{fontSize:11,color:"#3b82f6"}}>{c.time||""} • {c.frequency||"Semanal"}</div>
              </div>
            </div>
          ))}
        </Card>
      ...
      {birthdays.length>0&&(
        <Card style={{marginBottom:14,border:"1.5px solid #fde68a",background:"#fffbeb"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            <span style={{fontSize:14,fontWeight:800,color:"#92400e"}}>🎂 Aniversariantes do Mês</span>
          </div>
          {birthdays.map(m=>(
            <div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderTop:"1px solid #fde68a"}}>
              <Avatar name={m.name} photo={m.photo_url} size={28} color="#d97706"/>
              <div><div style={{fontSize:13,fontWeight:700,color:"#92400e"}}>{m.name}</div>
...
function CellsPanel({session,showToast}){
  ...
  const emptyForm={name:"",day:"Quarta",time:"19:30",neighborhood:"",street:"",number:"",cep:"",leader_id:"",secretary_id:"",host_id:"",supervisor_id:"",started_at:"",growth_goal:"",active:true,frequency:"Semanal",reminder_hours:24,reminder_channels:[],auto_create_meetings:true}
  const[form,setForm]=useState(emptyForm)
  const f=k=>v=>setForm(p=>({...p,[k]:v}))

  function openEdit(c){
    setForm({name:c.name,day:c.day||"Quarta",time:c.time||"",neighborhood:c.neighborhood||"",street:c.street||"",number:c.number||"",cep:c.cep||"",leader_id:c.leader_id||"",secretary_id:c.secretary_id||"",host_id:c.host_id||"",supervisor_id:c.supervisor_id||"",started_at:c.started_at||"",growth_goal:c.growth_goal||"",active:c.active!==false,frequency:c.frequency||"Semanal",reminder_hours:c.reminder_hours||24,reminder_channels:c.reminder_channels||[],auto_create_meetings:c.auto_create_meetings!==false})
  ...
  async function save(){
    ...
    const nextDate=form.auto_create_meetings?getNextMeetingDate(form.day,form.frequency):null
    const payload={name:form.name.trim().toUpperCase(),day:form.day,time:form.time,neighborhood:form.neighborhood,street:form.street,number:form.number,cep:form.cep,leader_id:form.leader_id||null,secretary_id:form.secretary_id||null,host_id:form.host_id||null,supervisor_id:form.supervisor_id||null,started_at:form.started_at||null,growth_goal:parseInt(form.growth_goal)||0,active:form.active,frequency:form.frequency,reminder_hours:parseInt(form.reminder_hours)||24,reminder_channels:form.reminder_channels,auto_create_meetings:form.auto_create_meetings,next_meeting_date:nextDate}
    if(editing){
  ...
  return(
    <div>
      ...
      {cells.map(cell=>{
        ...
        const host=members.find(m=>m.id===cell.host_id)
        ...
        return(
          <Card key={cell.id} style={{marginBottom:10,opacity:isActive?1:0.6,border:isActive?"1.5px solid #f1f5f9":"1.5px solid #fecaca"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:12,color:"#64748b",marginBottom:8}}>
              ...
              <span>🏠 Anfitrião: <b style={{color:"#334155"}}>{host?.name||"—"}</b></span>
              <span>👥 Membros: <b style={{color:"#334155"}}>{mc}</b></span>
            </div>
            {cell.growth_goal>0&&<div style={{marginTop:8}}><ProgressBar value={mc} max={cell.growth_goal} color="#2563eb"/></div>}
            {cell.started_at&&<div style={{fontSize:11,color:"#94a3b8",marginTop:8}}>📅 Iniciada em: {fmtDate(cell.started_at)}</div>}
            {cell.reminder_channels?.length>0&&<div style={{fontSize:11,color:"#7dd3fc",marginTop:4}}>🔔 Lembrete {cell.reminder_hours}h antes via {cell.reminder_channels.join(", ")}</div>}
          </Card>
        )
      })}
      <Modal open={modal} onClose={()=>setModal(false)} title={editing?"Editar Célula":"Nova Célula"}>
        ...
        <div style={{marginBottom:14}}>
          <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
            <input type="checkbox" checked={form.auto_create_meetings} onChange={e=>f("auto_create_meetings")(e.target.checked)} style={{width:16,height:16}}/>
            <span style={{fontSize:13,fontWeight:600,color:"#334155"}}>Criar encontros automaticamente</span>
          </label>
        </div>
        ...
        <div style={{marginBottom:14,background:"#f0fdf4",border:"1.5px solid #bbf7d0",borderRadius:12,padding:14}}>
          ...
          <p style={{fontSize:11,fontWeight:700,color:"#64748b",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.04em"}}>Canais</p>
          <div style={{display:"flex",gap:8}}>
            {["SMS","WhatsApp","Email"].map(ch=>(
              <button key={ch} type="button" onClick={()=>toggleChannel(ch)} style={{flex:1,padding:"8px 4px",borderRadius:10,fontSize:12,fontWeight:700,border:`1.5px solid ${form.reminder_channels.includes(ch)?"#059669":"#e2e8f0"}`,background:form.reminder_channels.includes(ch)?"#dcfce7":"#f8fafc",color:form.reminder_channels.includes(ch)?"#166534":"#64748b",cursor:"pointer"}}>
                {ch==="SMS"?"📱":ch==="WhatsApp"?"💬":"📧"} {ch}
              </button>
            ))}
          </div>
          <p style={{fontSize:11,color:"#94a3b8",marginTop:8,marginBottom:0}}>⚡ Envio automático será ativado em breve</p>
        </div>
        ...
        <div style={{marginBottom:14}}>
          <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
            ...
            <span style={{fontSize:13,fontWeight:600,color:"#334155"}}>Célula ativa</span>
          </label>
        </div>
...
function MembersPanel({session,showToast}){
  ...
  const emptyForm={name:"",cpf:"",birth_date:"",age:"",gender:"Masculino",phone:"",email:"",neighborhood:"",cell_id:"",status:"Visitante",role:"member",roles:[],baptized:false,baptism_date:"",invited_by:"",father_name:"",mother_name:"",spouse_name:"",photo_url:""}
  const[form,setForm]=useState(emptyForm)
  const f=k=>v=>setForm(p=>({...p,[k]:v}))
  function openEdit(m){
    setForm({name:m.name,cpf:m.cpf||"",birth_date:m.birth_date||"",age:m.age||"",gender:m.gender||"Masculino",phone:m.phone||"",email:m.email||"",neighborhood:m.neighborhood||"",cell_id:m.cell_id||"",status:m.status||"Visitante",role:m.role||"member",roles:m.roles||[],baptized:m.baptized||false,baptism_date:m.baptism_date||"",invited_by:m.invited_by||"",father_name:m.father_name||"",mother_name:m.mother_name||"",spouse_name:m.spouse_name||"",photo_url:m.photo_url||""})
    setEditing(m.id);setModal(true)
  }
  function toggleRole(role){setForm(p=>({...p,roles:p.roles.includes(role)?p.roles.filter(r=>r!==role):[...p.roles,role]}))}
  function openFamilySearch(field){setFamilyField(field);setFamilySearch("");setShowFamilySearch(true)}
  function selectFamilyMember(m){setForm(p=>({...p,[familyField]:m.name}));setShowFamilySearch(false)}
  const filteredFamily=members.filter(m=>m.name.toLowerCase().includes(familySearch.toLowerCase())&&m.id!==editing)
  async function save(){
    if(!form.name.trim()){showToast("Nome é obrigatório","error");return}
    const cpfNorm=form.cpf.replace(/\D/g,"")
    const payload={name:form.name.trim().toUpperCase(),cpf:cpfNorm,age:form.birth_date?calcAge(form.birth_date):(parseInt(form.age)||0),birth_date:form.birth_date||null,gender:form.gender,phone:form.phone,email:form.email,neighborhood:form.neighborhood,cell_id:form.cell_id||null,status:form.status,role:form.role,roles:form.roles,baptized:form.baptized,baptism_date:form.baptized&&form.baptism_date?form.baptism_date:null,invited_by:form.invited_by,father_name:form.father_name,mother_name:form.mother_name,spouse_name:form.spouse_name,photo_url:form.photo_url}
    if(editing){
      ...
      if(newM){
        ...
        await supabase.from("users").insert({member_id:newM.id,cpf:cpfLogin,password_hash:btoa64("123456"),name:form.name.trim().toUpperCase(),role:form.role,cell_id:form.cell_id||null,active:true})
        ...
        showToast("Membro criado! Senha: 123456")
      }
    }
    setModal(false);setEditing(null);setForm(emptyForm)
  }
  ...
  async function toggleActive(m){
    ...
    await supabase.from("members").update({status:isActive?"Afastado":"Membro"}).eq("id",m.id)
  ...
  return(
    <div>
        ...
        return(
          <Card key={m.id} style={{marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
              <Avatar name={m.name} photo={m.photo_url} size={40} color={ROLE_COLORS[m.role]||"#64748b"}/>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
              ...
              {m.roles?.map(r=><Badge key={r} label={r} color="#7c3aed"/>)}
            ...
            <div style={{display:"flex",gap:6,justifyContent:"flex-end",flexWrap:"wrap"}}>
              ...
              {session?.role==="admin"&&<button onClick={()=>{setPwModal(m.id);setNewPw("")}} style={{background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"5px 8px",cursor:"pointer",color:"#64748b",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",gap:3}}><Icon name="key" size={12}/>Senha</button>}
              ...
              <button onClick={()=>toggleActive(m)} style={{background:m.status==="Afastado"?"#dcfce7":"#fef3c7",border:"none",borderRadius:8,padding:6,cursor:"pointer",color:m.status==="Afastado"?"#059669":"#d97706"}}><Icon name={m.status==="Afastado"?"play":"pause"} size={14}/></button>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editing?"Editar Membro":"Novo Membro"}>
        ...
        <Sel label="Perfil Principal" value={form.role} onChange={f("role")} options={rOpts}/>
        <div style={{marginBottom:14}}>
          <label style={{display:"block",fontSize:11,fontWeight:700,color:"#64748b",marginBottom:8,letterSpacing:"0.05em",textTransform:"uppercase"}}>Funções Adicionais</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{MULTI_ROLES.map(r=><button key={r} type="button" onClick={()=>toggleRole(r)} style={{padding:"5px 10px",borderRadius:20,fontSize:12,fontWeight:600,border:`1.5px solid ${form.roles.includes(r)?"#7c3aed":"#e2e8f0"}`,background:form.roles.includes(r)?"#ede9fe":"#f8fafc",color:form.roles.includes(r)?"#7c3aed":"#64748b",cursor:"pointer"}}>{r}</button>)}</div>
        </div>
          ...
          {[["father_name","Pai"],["mother_name","Mãe"],["spouse_name","Cônjuge"]].map(([field,label])=>(
            <div key={field} style={{marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button type="button" onClick={()=>openFamilySearch(field)} style={{background:"#eff6ff",border:"none",borderRadius:10,padding:"10px 12px",cursor:"pointer",color:"#2563eb",display:"flex",alignItems:"center"}}><Icon name="search" size={14}/></button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={showFamilySearch} onClose={()=>setShowFamilySearch(false)} title="Buscar Membro">
        <div style={{position:"relative",marginBottom:12}}>
          <div style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#94a3b8",pointerEvents:"none"}}><Icon name="search" size={15}/></div>
          <input value={familySearch} onChange={e=>setFamilySearch(e.target.value)} placeholder="Digite o nome..." autoFocus style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:10,padding:"10px 14px 10px 36px",fontSize:14,outline:"none"}}/>
        </div>
        {filteredFamily.slice(0,10).map(m=>(
          <button key={m.id} onClick={()=>selectFamilyMember(m)} style={{width:"100%",textAlign:"left",background:"#f8fafc",border:"1.5px solid #e2e8f0",borderRadius:10,padding:"10px 14px",cursor:"pointer",marginBottom:6,display:"flex",alignItems:"center",gap:10}}>
            <Avatar name={m.name} photo={m.photo_url} size={32} color={ROLE_COLORS[m.role]||"#64748b"}/>
      ...
      {deleteId&&<Modal open title="Confirmar Exclusão" onClose={()=>setDeleteId(null)}>
        <p style={{color:"#64748b",marginBottom:16}}>Remover permanentemente?</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <Btn variant="ghost" onClick={()=>setDeleteId(null)}>Cancelar</Btn>
          <Btn variant="danger" onClick={del}>Excluir</Btn>
        </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.8)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:340,overflow:"hidden"}}>
        <div style={{background:"linear-gradient(135deg,#1e40af,#1e3a5f)",padding:"24px 20px",textAlign:"center"}}>
          ...
          <div style={{color:"#bfdbfe",fontSize:12}}>{ROLES[member.role]||"Membro"}</div>
          {member.roles?.length>0&&<div style={{color:"#93c5fd",fontSize:11,marginTop:4}}>{member.roles.join(" • ")}</div>}
        </div>
        <div style={{padding:"16px 20px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            <div style={{textAlign:"center",background:"#f8fafc",borderRadius:10,padding:10}}>
              <div style={{fontSize:11,color:"#94a3b8",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.04em"}}>Célula</div>
              <div style={{fontSize:13,fontWeight:800,color:"#0f172a",marginTop:2}}>{cell?.name||"—"}</div>
            </div>
            <div style={{textAlign:"center",background:"#f8fafc",borderRadius:10,padding:10}}>
              <div style={{fontSize:11,color:"#94a3b8",fontWeight:600,textTransform:"uppercase",letterSpacing:"0.04em"}}>Status</div>
              <div style={{fontSize:13,fontWeight:800,color:"#059669",marginTop:2}}>{member.status}</div>
            </div>
          </div>
...
function AttendancePanel({session,showToast}){
  ...
  const cellMembers=members.filter(m=>m.cell_id===cellId&&m.status!=="Afastado")
  ...
  async function save(){
    ...
    if(cell?.auto_create_meetings){
      const nextDate=getNextMeetingDate(cell.day,cell.frequency||"Semanal")
  ...
  return(
    <div>
      ...
      <Card style={{marginBottom:14}}>
        <Sel label="Célula" value={cellId} onChange={v=>{setCellId(v);setMarks({})}} options={[{value:"",label:"Selecione a célula..."},...cells.filter(c=>c.active!==false).map(c=>({value:c.id,label:`${c.name}${c.next_meeting_date?` • próx: ${fmtDate(c.next_meeting_date)}`:""}`}))]}/>
        ...
        <Inp label="Quem passou a Palavra" value={preacher} onChange={setPreacher} placeholder="Nome do pregador"/>
...
function CommentsModal({date,cellId,session,showToast,onClose}){
  ...
  async function addComment(){
    ...
    setNewComment("");setReaction("");reload()
    showToast("Comentário adicionado!")
  }
  return(
    <Modal open title={`Comentários • ${fmtDate(date)}`} onClose={onClose}>
      <div style={{marginBottom:16}}>
        <p style={{fontSize:13,fontWeight:700,color:"#0f172a",marginBottom:8}}>Adicionar comentário:</p>
        ...
        <div style={{marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {REACTIONS.map(r=>(
              <button key={r} onClick={()=>setReaction(r===reaction?"":r)} style={{fontSize:20,background:reaction===r?"#eff6ff":"#f8fafc",border:`1.5px solid ${reaction===r?"#2563eb":"#e2e8f0"}`,borderRadius:10,padding:"6px 10px",cursor:"pointer"}}>{r}</button>
            ))}
          </div>
        </div>
        <Btn full onClick={addComment} icon="send">Comentar</Btn>
      </div>
      <div style={{borderTop:"1.5px solid #f1f5f9",paddingTop:14}}>
        ...
        {cellComments.length===0&&<p style={{color:"#94a3b8",fontSize:13,textAlign:"center"}}>Nenhum comentário ainda. Seja o primeiro! 😊</p>}
        {cellComments.map(c=>(
          <div key={c.id} style={{background:"#f8fafc",borderRadius:12,padding:"10px 14px",marginBottom:8,border:"1.5px solid #f1f5f9"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                <span style={{fontSize:11,color:"#94a3b8"}}>{new Date(c.created_at).toLocaleString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</span>
              </div>
            </div>
            <p style={{fontSize:13,color:"#475569",margin:0,lineHeight:1.5}}>{c.comment}</p>
          </div>
        ))}
      </div>
    </Modal>
...
function EventsPanel({session,showToast}){
  ...
  async function save(){
    ...
    await addLog(session,"create",`Evento: ${form.title}`)
    setModal(false);setEditing(null);setForm(emptyForm)
  }
  ...
  return(
    <div>
        ...
        return(
          <Card key={ev.id} style={{marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
              <div>
                <div style={{fontSize:15,fontWeight:800,color:"#0f172a"}}>{ev.title}</div>
                <div style={{fontSize:12,color:"#64748b",marginTop:2}}>{fmtDate(ev.date)}{ev.time&&` às ${ev.time}`}</div>
                {ev.location&&<div style={{fontSize:12,color:"#64748b"}}>📍 {ev.location}</div>}
                {cell&&<div style={{fontSize:12,color:"#64748b"}}>🏠 {cell.name}</div>}
              </div>
              <div style={{display:"flex",gap:4,flexDirection:"column",alignItems:"flex-end"}}>
                <Badge label={ev.type} color="#7c3aed"/>
                {attCount>0&&<Badge label={`✓ ${attCount}`} color="#059669"/>}
              </div>
            </div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title={editing?"Editar Evento":"Novo Evento"}>
        ...
        <Sel label="Célula (opcional)" value={form.cell_id} onChange={f("cell_id")} options={cOpts}/>
      ...
      {attModal&&(
        <Modal open title={`Presença — ${attModal.title}`} onClose={()=>{setAttModal(null);setMarks({})}}>
          {(attModal.cell_id?members.filter(m=>m.cell_id===attModal.cell_id):members).map(m=>{
            const s=marks[m.id]
            return(
              <div key={m.id} style={{marginBottom:8}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {["Presente","Ausente"].map(v=>(
                    <button key={v} onClick={()=>setMarks(p=>({...p,[m.id]:v}))} style={{padding:"6px",borderRadius:8,fontSize:12,fontWeight:700,border:`1.5px solid ${s===v?(v==="Presente"?"#059669":"#dc2626"):"#e2e8f0"}`,background:s===v?(v==="Presente"?"#dcfce7":"#fee2e2"):"#f8fafc",color:s===v?(v==="Presente"?"#166534":"#991b1b"):"#64748b",cursor:"pointer"}}>
...
function ReportsPanel(){
  ...
  const cellData=cells.map(c=>({name:c.name,count:members.filter(m=>m.cell_id===c.id).length,goal:c.growth_goal||0,active:c.active!==false})).sort((a,b)=>b.count-a.count)
  const baptized=members.filter(m=>m.baptized).length
  const currentMonth=getCurrentMonth()
  const birthdays=members.filter(m=>m.birth_date&&getMonthBirthday(m.birth_date)===currentMonth)
  const attByMember={}
  ...
  return(
    <div>
      ...
      {birthdays.length>0&&(
        <Card style={{marginBottom:12,border:"1.5px solid #fde68a",background:"#fffbeb"}}>
          <h3 style={{fontSize:14,fontWeight:800,color:"#92400e",marginBottom:10}}>🎂 Aniversariantes do Mês</h3>
          {birthdays.map(m=>(<div key={m.id} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderTop:"1px solid #fde68a"}}><Avatar name={m.name} photo={m.photo_url} size={28} color="#d97706"/><div><div style={{fontSize:13,fontWeight:700,color:"#92400e"}}>{m.name}</div><div style={{fontSize:11,color:"#b45309"}}>{fmtDate(m.birth_date)}</div></div></div>))}
        </Card>
      )}
      <Card style={{marginBottom:12}}>
        <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",marginBottom:12}}>🎯 Meta de Crescimento por Célula</h3>
        {cellData.map(({name,count,goal,active})=>(
          <div key={name} style={{marginBottom:12,opacity:active?1:0.5}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
              <span style={{fontSize:12,fontWeight:700,color:"#334155"}}>{name}{!active&&" (inativa)"}</span>
              <span style={{fontSize:12,color:"#64748b"}}>{count}{goal>0?`/${goal}`:""}</span>
            </div>
            {goal>0?<ProgressBar value={count} max={goal} color="#2563eb"/>:<div style={{height:8,background:"#f1f5f9",borderRadius:4}}/>}
          </div>
        ))}
      </Card>
      {[{title:"Por Sexo",data:genderData,color:"#2563eb"},{title:"Por Status",data:statusData,color:"#059669"}].map(({title,data,color})=>(
        <Card key={title} style={{marginBottom:12}}>
          ...
          {Object.entries(data).map(([k,v])=>(<div key={k} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><span style={{fontSize:12,fontWeight:600,color:"#334155",minWidth:90}}>{k}</span><Bar value={v} max={members.length} color={color}/><span style={{fontSize:12,fontWeight:700,color,minWidth:24,textAlign:"right"}}>{v}</span></div>))}
        </Card>
      ))}
      {attList.length>0&&(
        <Card>
...
function MessagesPanel({session,showToast}){
  ...
  function getTargetCount(){
    if(form.target_type==="all")return members.length
    if(form.target_type==="cell"&&form.target_cell_id)return members.filter(m=>m.cell_id===form.target_cell_id).length
  ...
  async function send(){
    ...
    await addLog(session,"create",`Mensagem: ${form.title}`)
    showToast(`Mensagem registrada para ${getTargetCount()} pessoas!`)
    setForm(p=>({...p,title:"",body:""}))
  }
  const myCells=isSupervisor?cells.filter(c=>c.supervisor_id===session.member_id):cells
  const cOpts=(isAdmin?cells:myCells).map(c=>({value:c.id,label:c.name}))
  return(
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {form.target_type==="cell"&&<Sel label="Célula" value={form.target_cell_id} onChange={f("target_cell_id")} options={[{value:"",label:"Selecione..."},...cOpts]}/>}
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {["sms","whatsapp","email"].map(ch=>(<button key={ch} onClick={()=>setForm(p=>({...p,channels:{...p.channels,[ch]:!p.channels[ch]}}))} style={{flex:1,padding:"10px 6px",borderRadius:10,border:`1.5px solid ${form.channels[ch]?"#2563eb":"#e2e8f0"}`,background:form.channels[ch]?"#eff6ff":"#f8fafc",color:form.channels[ch]?"#1e40af":"#94a3b8",fontSize:12,fontWeight:700,cursor:"pointer"}}>{ch==="sms"?"📱 SMS":ch==="whatsapp"?"💬 WhatsApp":"📧 E-mail"}</button>))}
          </div>
        </div>
        {getTargetCount()>0&&<div style={{background:"#eff6ff",borderRadius:10,padding:"8px 12px",marginBottom:12,fontSize:13,color:"#1e40af",fontWeight:600}}>📤 Será registrado para {getTargetCount()} pessoa(s)</div>}
...
function RequestsPanel({session,showToast}){
  ...
  async function resolve(id,status){
    ...
    if(status==="approved"){const req=requests.find(r=>r.id===id);if(req){await supabase.from("members").update({status:"Afastado"}).eq("id",req.member_id);await supabase.from("users").update({active:false}).eq("member_id",req.member_id)}}
  ...
  const mOpts=[{value:"",label:"Selecione o membro..."},...members.map(m=>({value:m.id,label:m.name}))]
  return(
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      ...
      {requests.length===0&&<Card><p style={{color:"#94a3b8",textAlign:"center",margin:0}}>Nenhuma solicitação</p></Card>}
      {requests.map(r=>(<Card key={r.id} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div><span style={{fontSize:14,fontWeight:800,color:"#0f172a"}}>{r.member_name}</span><div style={{fontSize:11,color:"#94a3b8",marginTop:2}}>Por: {r.requested_by_name}</div></div><Badge label={sl[r.status]||r.status} color={sc[r.status]||"#64748b"}/></div><p style={{fontSize:12,color:"#64748b",margin:"0 0 8px"}}>Motivo: {r.reason}</p><div style={{fontSize:11,color:"#94a3b8",marginBottom:8}}>{fmtDate(r.created_at?.split("T")[0])}</div>{r.status==="pending"&&canResolve&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Btn variant="success" size="sm" onClick={()=>resolve(r.id,"approved")}>✓ Aprovar</Btn><Btn variant="danger" size="sm" onClick={()=>resolve(r.id,"rejected")}>✗ Rejeitar</Btn></div>)}</Card>))}
      <Modal open={modal} onClose={()=>setModal(false)} title="Nova Solicitação de Inativação">
        ...
        <Btn full onClick={createRequest}>Enviar Solicitação</Btn>
      </Modal>
    </div>
  )
}
...
function SubDashboard({session,logout,showToast,title,subtitle,menu,stats,extraContent}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      <header style={{background:"#0f172a",padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div><div style={{color:"#f0f9ff",fontSize:16,fontWeight:800}}>{title}</div><div style={{color:"#7dd3fc",fontSize:12}}>{subtitle}</div></div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowChangePw(true)} style={{background:"rgba(255,255,255,0.08)",border:"1.5px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"7px 10px",cursor:"pointer",color:"#cbd5e1",display:"flex"}}><Icon name="key" size={15}/></button>
          <button onClick={logout} style={{background:"rgba(255,255,255,0.08)",border:"1.5px solid rgba(255,255,255,0.12)",borderRadius:10,padding:"7px 12px",cursor:"pointer",color:"#cbd5e1",display:"flex",alignItems:"center",gap:6,fontSize:13,fontWeight:600}}><Icon name="log-out" size={15}/>Sair</button>
        </div>
      ...
      {sub==="home"?(
        <div style={{flex:1,padding:"16px 16px 80px"}}>
          {stats&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>{stats}</div>}
          {extraContent}
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {menu.map(item=>(
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"16px 16px 80px"}}>
            {menu.find(i=>i.id===sub)?.component}
          </div>
        </div>
      )}
      <ChangePasswordModal open={showChangePw} onClose={()=>setShowChangePw(false)} session={session} showToast={showToast}/>
...
function SecretaryDashboard({session,logout,showToast}){
  ...
  const menu=[
    {id:"members",icon:"users",label:"Membros",desc:"Gerenciar cadastros",color:"#059669",component:<MembersPanel session={session} showToast={showToast}/>},
    {id:"attendance",icon:"check-circle",label:"Presença",desc:"Registrar reunião",color:"#2563eb",component:<AttendancePanel session={session} showToast={showToast}/>},
    {id:"events",icon:"event",label:"Eventos",desc:"Gerenciar eventos",color:"#7c3aed",component:<EventsPanel session={session} showToast={showToast}/>},
    {id:"reports",icon:"bar-chart",label:"Relatórios",desc:"Frequência e dados",color:"#d97706",component:<ReportsPanel/>},
    {id:"messages",icon:"message",label:"Mensagens",desc:"Enviar comunicados",color:"#0891b2",component:<MessagesPanel session={session} showToast={showToast}/>},
    {id:"requests",icon:"inbox",label:"Solicitações",desc:"Inativações",color:"#dc2626",component:<RequestsPanel session={session} showToast={showToast}/>},
  ]
  const stats=(<><Stat label="Membros" value={cellMembers.length} color="#059669" icon="users"/><Stat label="Presentes (últ.)" value={presentCount} color="#2563eb" icon="check-circle"/></>)
  const extra=cell?.growth_goal>0?(<Card style={{marginBottom:16}}><div style={{fontSize:13,fontWeight:700,color:"#334155",marginBottom:8}}>🎯 Meta de Crescimento</div><ProgressBar value={cellMembers.length} max={cell.growth_goal} color="#2563eb"/></Card>):null
  return<SubDashboard session={session} logout={logout} showToast={showToast} title={cell?.name||"Minha Célula"} subtitle="Secretário" menu={menu} stats={stats} extraContent={extra}/>
}
...
function LeaderDashboard({session,logout,showToast}){
  ...
  const MembersView=()=>(<div>{cellMembers.length===0&&<Card><p style={{color:"#94a3b8",textAlign:"center",margin:0}}>Nenhum membro</p></Card>}{cellMembers.map(m=>(<Card key={m.id} style={{marginBottom:8}}><div style={{display:"flex",alignItems:"center",gap:12}}><Avatar name={m.name} photo={m.photo_url} size={36} color={ROLE_COLORS[m.role]||"#64748b"}/><div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:"#0f172a"}}>{m.name}</div><div style={{fontSize:12,color:"#94a3b8"}}>{m.phone||"—"} • {m.status}</div>{m.roles?.length>0&&<div style={{fontSize:11,color:"#7c3aed",marginTop:2}}>{m.roles.join(" • ")}</div>}</div>{m.baptized&&<Badge label="Batizado" color="#d97706"/>}</div></Card>))}</div>)
  const FreqView=()=>(<div>{Object.values(attByM).length===0&&<Card><p style={{color:"#94a3b8",textAlign:"center",margin:0}}>Nenhum dado</p></Card>}{Object.values(attByM).sort((a,b)=>(b.present/b.total)-(a.present/a.total)).map(({name,total,present})=>{const pct=Math.round(present/total*100);const color=pct>=75?"#059669":pct>=50?"#d97706":"#dc2626";return(<Card key={name} style={{marginBottom:8}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:13,fontWeight:700,color:"#0f172a"}}>{name}</span><span style={{fontSize:14,fontWeight:800,color}}>{pct}%</span></div><div style={{height:6,background:"#f1f5f9",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:color,borderRadius:3}}/></div><div style={{fontSize:11,color:"#94a3b8",marginTop:4}}>{present}/{total} encontros</div></Card>)})}</div>)
  const menu=[
    {id:"members",icon:"users",label:"Membros",desc:"Ver todos",color:"#2563eb",component:<MembersView/>},
    {id:"frequency",icon:"bar-chart",label:"Frequência",desc:"Relatórios",color:"#059669",component:<FreqView/>},
    {id:"messages",icon:"message",label:"Mensagens",desc:"Enviar comunicados",color:"#7c3aed",component:<MessagesPanel session={session} showToast={showToast}/>},
    {id:"requests",icon:"inbox",label:"Solicitações",desc:"Inativações",color:"#dc2626",component:<RequestsPanel session={session} showToast={showToast}/>},
  ]
  const stats=(<><Stat label="Membros" value={cellMembers.length} color="#2563eb" icon="users"/><Stat label="Encontros" value={[...new Set(cellAtt.map(a=>a.date))].length} color="#059669" icon="calendar"/></>)
  const extra=cell?.growth_goal>0?(<Card style={{marginBottom:16}}><div style={{fontSize:13,fontWeight:700,color:"#334155",marginBottom:8}}>🎯 Meta de Crescimento</div><ProgressBar value={cellMembers.length} max={cell.growth_goal} color="#2563eb"/></Card>):null
  return<SubDashboard session={session} logout={logout} showToast={showToast} title={cell?.name||"Minha Célula"} subtitle="Líder" menu={menu} stats={stats} extraContent={extra}/>
}
...
function SupervisorDashboard({session,logout,showToast}){
  ...
  const pendingRequests=requests.filter(r=>r.status==="pending"&&supervised.some(c=>c.id===r.cell_id))
  const CellsView=()=>(<div>{supervised.length===0&&<Card><p style={{color:"#94a3b8",textAlign:"center",margin:0}}>Nenhuma célula atribuída</p></Card>}{supervised.map(cell=>{const mc=members.filter(m=>m.cell_id===cell.id);const leader=members.find(m=>m.id===cell.leader_id);return(<Card key={cell.id} style={{marginBottom:10}}><div style={{fontSize:15,fontWeight:800,color:"#0f172a",marginBottom:4}}>{cell.name}</div><div style={{fontSize:12,color:"#64748b",marginBottom:8}}>{cell.neighborhood} • {cell.day} às {cell.time}</div>{cell.frequency&&<div style={{fontSize:11,color:"#7c3aed",marginBottom:6}}>🔄 {cell.frequency}{cell.next_meeting_date?` • Próximo: ${fmtDate(cell.next_meeting_date)}`:""}</div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:12,color:"#64748b",marginBottom:8}}><span>👤 <b style={{color:"#334155"}}>{leader?.name||"—"}</b></span><span>👥 <b style={{color:"#334155"}}>{mc.length} membros</b></span></div>{cell.growth_goal>0&&<ProgressBar value={mc.length} max={cell.growth_goal} color="#7c3aed"/>}</Card>)})}</div>)
  const menu=[
    {id:"cells",icon:"grid",label:"Células",desc:"Visualizar células",color:"#2563eb",component:<CellsView/>},
    {id:"requests",icon:"inbox",label:"Solicitações",desc:`${pendingRequests.length} pendente(s)`,color:"#dc2626",component:<RequestsPanel session={session} showToast={showToast}/>},
    {id:"messages",icon:"message",label:"Mensagens",desc:"Comunicar com líderes",color:"#7c3aed",component:<MessagesPanel session={session} showToast={showToast}/>},
  ]
  const stats=(<><Stat label="Células" value={supervised.length} color="#7c3aed" icon="grid"/><Stat label="Pendências" value={pendingRequests.length} color="#dc2626" icon="inbox"/></>)
  return<SubDashboard session={session} logout={logout} showToast={showToast} title="Supervisor" subtitle={session.name} menu={menu} stats={stats} extraContent={null}/>
}
...
function MemberPortal({session,logout,showToast}){
  ...
  const cellMembers=cell?members.filter(m=>m.cell_id===cell.id):[]
  const leader=cell?members.find(m=>m.id===cell.leader_id):null
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      <div style={{background:"linear-gradient(135deg,#0f172a,#1e3a5f)",padding:"24px 18px 16px"}}>
        ...
        <div style={{display:"flex",gap:2,background:"rgba(255,255,255,0.08)",borderRadius:12,padding:3}}>
          {[["dados","Meus Dados"],["celula","Minha Célula"],["presenca","Presença"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)} style={{flex:1,padding:"8px 4px",borderRadius:10,fontSize:12,fontWeight:700,border:"none",cursor:"pointer",background:tab===id?"#fff":"transparent",color:tab===id?"#1e40af":"#93c5fd",transition:"all 0.15s"}}>{label}</button>
          ))}
        </div>
      </div>
      <div style={{flex:1,padding:"16px 16px 80px",overflowY:"auto"}}>
          ...
          <div>
            {cell?(<>
              <Card style={{marginBottom:12}}>
                ...
                {[["Nome",cell.name],["Dia",cell.day],["Horário",cell.time],["Periodicidade",cell.frequency],["Próximo encontro",fmtDate(cell.next_meeting_date)],["Bairro",cell.neighborhood],["Endereço",cell.street?`${cell.street}, ${cell.number||"s/n"}`:"—"],["Líder",leader?.name],["Membros",cellMembers.length.toString()],["Início",fmtDate(cell.started_at)]].map(([k,v])=>v&&v!=="—"?(<div key={k} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #f1f5f9",fontSize:13}}><span style={{color:"#94a3b8",fontWeight:600}}>{k}</span><span style={{color:"#334155",fontWeight:700,textAlign:"right"}}>{v}</span></div>):null)}
              </Card>
        ...
        {tab==="presenca"&&(
          <div>
            {myAtt.length>0&&(
              <Card style={{marginBottom:12,background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"1.5px solid #bfdbfe"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700,color:"#1e40af"}}>Minha Frequência</span><span style={{fontSize:18,fontWeight:900,color:pct>=75?"#059669":pct>=50?"#d97706":"#dc2626"}}>{pct}%</span></div>
                <div style={{height:8,background:"rgba(255,255,255,0.5)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${pct}%`,background:pct>=75?"#059669":pct>=50?"#d97706":"#dc2626",borderRadius:4}}/></div>
                <div style={{fontSize:12,color:"#3b82f6",marginTop:6}}>{myAtt.filter(a=>a.status==="Presente").length} presenças de {myAtt.length} encontros</div>
              </Card>
            )}
                ...
                <Card key={a.id} style={{marginBottom:6}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      {a.theme&&<div style={{fontSize:11,color:"#64748b"}}>📖 {a.theme}</div>}
                      {a.preacher&&<div style={{fontSize:11,color:"#64748b"}}>🎤 {a.preacher}</div>}
                    </div>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                      <Badge label={a.status} color={sc}/>
                      {a.status==="Presente"&&(<button onClick={()=>setCommentsModal({date:a.date,cellId:a.cell_id})} style={{background:"#f0fdf4",border:"none",borderRadius:6,padding:"3px 8px",cursor:"pointer",color:"#059669",fontSize:11,fontWeight:600,display:"flex",alignItems:"center",gap:3}}><Icon name="comment" size={11}/>Comentar</button>)}
                    </div>
                  </div>
                </Card>
              )
